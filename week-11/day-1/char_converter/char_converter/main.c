...
/*
 * Convert row encoded characters into
 * column encoded format
 */
...
void main()
{
    for (int letter = 0; letter < ascii_size; letter++) {

        clean_temp();

        // Put into a temp array 7 * 5
        for (int row = 0; row < 7; row++) {
            for (int col = 0; col < 5; col++) {
                if(ascii_row[letter][row] & (1 << (4 - col)))
                    temp_letter[row][col] = 1;
                else
                    temp_letter[row][col] = 0;
            }
        }
        ...
        for (int col = 0; col < 5; col++) {
            int col_val = 0;
            for (int row = 0; row < 7; row++) {
                col_val += temp_letter[row][col] & (1 << row);
            }
            printf("0x%02x", col_val);
            if (col < 4) {
                printf(", ");
            }
        }
    ...
    // Print column encoded format
    for (int r = 0; r < maxrow; r++) {
        for (int c = 0; c < maxcol; c++) {
            printf("%d", temp_letter[r][c]);
        }
        printf("\n");
    }
